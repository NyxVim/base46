local M = {}
local g = vim.g
local uiconfig = require("nyxconfig").ui
local opts = require("nyxconfig").base46
...
M.toggle_theme = function()
  ...
  if opts.theme ~= themes[1] and opts.theme ~= themes[2] then
    vim.notify "Set your current theme to one of those mentioned in the theme_toggle table (nyxrc)"
    return
  end
  ...
  require("nyxvim.utils").replace_word('theme = "' .. old_theme, 'theme = "' .. opts.theme)
...
M.toggle_transparency = function()
  ...
  local old = dofile(vim.fn.stdpath "config" .. "/lua/nyxrc.lua").transparency
  local new = "transparency = " .. tostring(opts.transparency)
  require("nyxvim.utils").replace_word("transparency = " .. tostring(old), new)
end
